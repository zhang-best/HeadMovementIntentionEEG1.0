...
%% 加载工作态的小波变换图，一共为208次
    pathC = strcat('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\',name(namei),'\',name(namei),'wave300_50s.mat');
    C = load(pathC);
...
%% 合并二者
    ...
    for i = 1:trials
        for r = 1:10
            for j = 1:19
                for q = 1:factor
                    coefs_channel_trial(r+50,q*10-9:q*10,j,i) = mse_fuzzys(j,q,i);
                end
            end
        end
    end
%% 归一化20-38通道图的值【0，255】
    ...
    label300 = A.label300;
    pathD = strcat('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\',name(namei),'\',name(namei),'wavefuzzy300_50s.mat');
