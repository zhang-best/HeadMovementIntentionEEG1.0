%准确率94.5946%，样本数量最好大于100，25ms一次

clear;
load('C:\Users\user\Desktop\science research\转动意图识别\数据集\BCICIII_IVa_mat\data_set_IVa_al_mat\100Hz\data_set_IVa_al.mat');

%BCIcomp2002.mat文件转换，训练样本转换
for i = 1:150
    x_train(:,:,i) = 0.1*double(cnt(mrk.pos(1,i):mrk.pos(1,i)+350-1,:));
    y_train(i,1) = mrk.y(1,i);
end
for i = 1:74
    x_test(:,:,i) = 0.1*double(cnt(mrk.pos(1,i+150):mrk.pos(1,i+150)+350-1,:));
    y_test(i,1) = mrk.y(1,i+150);
end
...
sampleRate=100;%128
startTime= 0;
k=8;       %定义Mutual Select K values
freq=[4 8 12 16 20 24 28 32 36 40]; %设置子频带频率[7 12 16 20 24 28 30]
[FBtrainf,proj,classNum]=FBCSP(X_train(1:end,:,:),Y_train,sampleRate,CSPm,freq); %对训练集数据进行FBCSP
...
%% 训练模型
model=svmtrain(Y_train,selFeaTrain,'-c 2 -g 0.1250'); %训练模型
tic
fbtestf=FBCSPOnline(X_test(1:end,:,:),proj,classNum,sampleRate,CSPm,freq);  %对测试集数据进行FBCSP
selFeaTest=fbtestf(:,rank(1:k,2));                               %MuI test set features
% predict=svmpredict(Y_test,selFeaTest,model);
[predictlabel,ac,decv]=svmpredict(Y_test,selFeaTest,model);
ac_1=ac(1);
disp(ac_1);
toc  
