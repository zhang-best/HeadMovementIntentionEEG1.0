...
for i = 1:size(signal,3)
    for j = 1:size(signal,1)
        ...
        energys(j,i) = sum(min(abs(coefs)));
        z=control(j,:,i);%取得信号
        coefs = cwt(z,scal,wavename);
        energyc(j,i) = sum(min(abs(coefs)));%min(sum(abs(coefs),2))
    end
end
%% 直接进行前向融合
% for i =1:size(signal,3)
...
for i =1:150%生成训练集300=150空闲+150工作
    for j = 1:19
        X(j,i) = FuzzyEns(j,i);
        X(j+19,i) = energys(j,i);
        X(j,i+150) = FuzzyEnc(j,i);
        X(j+19,i+150) = energyc(j,i);
        classX(i,1) = 0;
        classX(i+150,1) = 1;
    end
end
...
for i =1:t%生成训练集114=57空闲+57工作
    for j = 1:19
        Y(j,i) = FuzzyEns(j,i+150);
        Y(j+19,i) = energys(j,i+150);
        Y(j,i+t) = FuzzyEnc(j,i+150);
        Y(j+19,i+t) = energyc(j,i+150);
        classY(i,1) = 0;
        classY(i+t,1) = 1;
    end
end
