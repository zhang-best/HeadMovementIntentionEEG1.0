...
%生成SVM所需的数据集
    for i =1:150%生成训练集300=150空闲+150工作
        for j = 1:19
            X(j,i) = mse_fuzzyc(j,index,i);
            X(j,i+150) = mse_fuzzys(j,index,i);
            classX(i,1) = 0;
            classX(i+150,1) = 1;
        end
    end
    ...
    for i =1:t%生成训练集114=57空闲+57工作
        for j = 1:19
            Y(j,i) = mse_fuzzyc(j,index,i+150);
            Y(j,i+t) = mse_fuzzys(j,index,i+150);
            classY(i,1) = 0;
            classY(i+t,1) = 1;
        end
    end
    Y = Y';
    [~,~,CLASS] = dclass(X(1:150,:),X(151:300,:),Y);
    num = 0;
    for i =1:2*t%生成训练集114=57空闲+57工作
        if(i<t+1 && CLASS(1,i)==1)
            num=num+1;
        end
        if(i>t && CLASS(1,i)==2)
            num=num+1;
...
clear;
A = load('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\lhy\lhydata300_50s.mat');
B = load('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\lhy\lhydata300control.mat');
signal = A.signal300;%19*300*208
control = B.control300;
dim = 2;
n = 2;
