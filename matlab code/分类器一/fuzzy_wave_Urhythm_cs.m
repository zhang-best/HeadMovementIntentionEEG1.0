...
%% 对分类器一产生输入数据：进行小波变换和fuzzy+urhythm计算的图片
    pathA = strcat('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\',name(namei),'\',name(namei),'wavefuzzy300cs.mat');
    A = load(pathA);
...
%% 计算u节律
    ...
    B = load(pathB);
    pathC = strcat('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\',name(namei),'\',name(namei),'data300control.mat');
    C = load(pathC);
    signal = B.signal300;%19*300*208
    control = C.control300;
    for i = 1:trials/2
        for j = 1:channels
            %小波分析时间频率图1-40HZ
...
%% 合并二者
    ...
    for i = 1:trials/2
        for r = 1:size(energyc,1)
            for j = 1:19
                coefs_channel_trial(r+68,:,j,i) = energyc(r,:,j,i);
                coefs_channel_trial(r+68,:,j,i + trials/2) = energys(r,:,j,i);
            end
        end
    end
%% 归一化20-38通道图的值【0，255】
    ...
    pathD = strcat('E:\science research\转动意图识别\laboratory_data&result\data\转头21_mat\',name(namei),'\',name(namei),'wavefuzzyUrhythm300cs.mat');
