...
for seedi in range(1, 2):
    ...
    path = r'E:/science research/转动意图识别/laboratory_data&result/data/转头21_mat/' + namei + '/' + namei + 'wave300cs.mat'
    data = h5py.File(path)
    # 读取mat文件
    ...
    for i in range(0, d):
        for j in range(0, c):
            for k in range(0, b):
                for h in range(0, a):
                    coefs_channel_trial[h, k, j, i] = coefs_channel_trial_temp[i, listc[j], k, h] / 255
    ...
    for train, val in kfold.split(B, A):
        ...
        model.add(Flatten())
        model.add(Dense(600, kernel_regularizer=keras.regularizers.l2(0.002),
                        activation='relu'))  # activity_regularizer=keras.regularizers.l1(0.002), , kernel_regularizer=regularizers.l2(2), activity_regularizer=regularizers.l1(2),利用正则化来减少过拟合现象
        model.add(Dropout(0.2))
        model.add(Dense(300, kernel_regularizer=keras.regularizers.l2(0.002),
                        activation='relu'))  # activity_regularizer=keras.regularizers.l1(0.002), , kernel_regularizer=regularizers.l2(2), activity_regularizer=regularizers.l1(2),利用正则化来减少过拟合现象
        model.add(Dense(2))
        # 使用softmax进行分类
