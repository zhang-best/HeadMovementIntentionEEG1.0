...
for namei in name:
    for seedi in range(1, 2):
        ...
        for train, val in kfold.split(B, A):
            ...
            f = open('C://Users/001/Desktop/2.txt', 'a', encoding='utf-8')  ##ffilename可以是原来的txt文件，也可以没有然后把写入的自动创建成txt文件
            f.write("%s:\n" % (namei))
            f.write("%s\n \n" % (H.history["val_loss"]))
            f.close()
            ...
            if (accuracy_now < accuracy):
                ...
                model.save('E:/pycharm projects/zhuantou3-3class/model/rwmodel3' + namei + '.h5')
