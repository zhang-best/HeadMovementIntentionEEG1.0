...
for namei in name:
    for seedi in range(1, 2):
        ...
        import gc
        path = r'E:/science research/转动意图识别/laboratory_data&result/data/转头21_mat/'+namei+'/'+namei+'wavefuzzy300cs.mat'
        data = h5py.File(path)
        # 读取mat文件
        ...
        for train, val in kfold.split(B, A):
            ...
            f = open('C://Users/001/Desktop/3fuzzy2.txt', 'a', encoding='utf-8')  ##ffilename可以是原来的txt文件，也可以没有然后把写入的自动创建成txt文件
            f.write("%s:\n" % (namei))
            f.write("%s\n \n" % (H.history["val_loss"]))
            f.close()
            ...
            if (accuracy_now < accuracy):
                ...
                model.save('E:/pycharm projects/zhuantou3-3class/model/fuzzyrwmodel3' + namei + '.h5')
            print('test before save:')  # 预测
            K.clear_session()
        print("%.2f%% (+/- %.2f%%)" % (np.mean(cvscores), np.std(cvscores)))
        f = open('C://Users/001/Desktop/3fuzzy1.txt', 'a', encoding='utf-8')  ##ffilename可以是原来的txt文件，也可以没有然后把写入的自动创建成txt文件
        f.write("%s:\n" % (namei))
        f.write("准确率：\n%s\n" % (cvscores))
