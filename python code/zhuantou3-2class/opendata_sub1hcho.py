for seedi in range(1, 2):
    ...
    for train, test in kfold.split(B, A):
        TRAIN = np.empty((trial // 10 * 9, num_f, timeline, channel))
        for i in range(0, trial // 10 * 9):
            TRAIN[i, :, :, :] = coefs[train[i], :, :, :]
        TEST = np.empty((trial // 10 * 1, num_f, timeline, channel))
        ...
        model.add(MaxPooling2D(
            pool_size=(5, 4),
            strides=(2, 2),
            border_mode='valid',  # padding method
        ))

        # 这是添加第二层神经网络，卷积层，激励函数，池化层
        model.add(Convolution2D(16, 3, 3, border_mode='same'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D(pool_size=(6, 5), strides=(2, 2), border_mode='valid'))  # size=6.3

        # 这是添加第三层神经网络，卷积层，激励函数，池化层

        model.add(Convolution2D(32, 3, 3, border_mode='same'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D(pool_size=(6, 6), strides=(2, 2), border_mode='valid'))  # size=6.4
        model.add(Flatten())
        model.add(Dense(
            1000))  # , kernel_regularizer=regularizers.l2(2), activity_regularizer=regularizers.l1(2),利用正则化来减少过拟合现象
        model.add(Dense(500))
        model.add(Dense(2))
        # 使用softmax进行分类
        ...
        adam = Adam(lr=2e-7)  # alldata lr=3e-7
        ...
        print('\nTraining-----------')
        H = model.fit(TRAIN, TRAIN_label300_class, nb_epoch=200,
                      batch_size=4)  # 测试的间隔次数为20,validation_split=0.2,,  validation_freq=1
        print('\nTesting------------')
...
for seedi in range(1, 2):
    ...
    path = r'E:\science research\转动意图识别\公开数据集\15sub-6move\S01_ME_mat\hcho\hchowavefuzzy.mat'
    data = h5py.File(path)
    # 读取mat文件
    # 读取mat文件中所有数据存储到array中
    coefs_channel_trial_temp = data['coefs_channel_trial'][:]  # 定义矩阵
...
for seedi in range(1, 2):
    ...
    path = r'E:\science research\转动意图识别\公开数据集\15sub-6move\S01_ME_pref\pref\prefwavefuzzy.mat'
    data = h5py.File(path)
    # 读取mat文件
    # 读取mat文件中所有数据存储到array中
    coefs_channel_trial_temp = data['coefs_channel_trial'][:]  # 定义矩阵
...
for seedi in range(1, 2):
    ...
    path = r'E:\science research\转动意图识别\公开数据集\15sub-6move\S01_ME_pref\pref\prefwave.mat'
    data = h5py.File(path)
    # 读取mat文件
    # 读取mat文件中所有数据存储到array中
    coefs_channel_trial_temp = data['coefs_channel_trial'][:]  # 定义矩阵
