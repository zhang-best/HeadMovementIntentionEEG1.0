name = ['wyh', 'xy', 'yyb', 'zc', 'zxj', 'zy', 'zzh']  #'fww', 'lc', 'lhy', 'lyb', 'wy',
for namei in name:
    for seedi in range(1, 2):
        ...
        import gc
        path = r'E:/science research/转动意图识别/laboratory_data&result/data/转头21_mat/'+namei+'/'+namei+'wave3DCNN_s.mat'
        data = h5py.File(path)
        # 读取mat文件
        ...
        for train, test in kfold.split(B, A):
            ...
            model.add(Activation('relu'))
            model.add(MaxPooling3D(pool_size=(1, 1, 6), strides=(1, 1, 3), border_mode='valid'))  # size=6.3
            ...
            model.add(Flatten())
            model.add(Dense(600))  # , kernel_regularizer=regularizers.l2(2), activity_regularizer=regularizers.l1(2),利用正则化来减少过拟合现象1024-512-150-59%
            model.add(Activation('relu'))
            ...
            H = model.fit(TRAIN, TRAIN_label300_class, nb_epoch=100,
                          batch_size=6)  # 测试的间隔次数为20,validation_split=0.2,,  validation_freq=1
            print('\nTesting------------')
